...
void B2500BinarySensorV2::on_message(B2500Message message) {
  ...
  if (message == B2500_MSG_TIMER_INFO) {
    ...
    if (this->smart_meter_connected_binary_sensor_ != nullptr &&
        this->smart_meter_connected_binary_sensor_->state != payload.base.smart_meter.connected) {
      this->smart_meter_connected_binary_sensor_->publish_state(payload.base.smart_meter.connected);
    }
  }
}
