...
const BUILD_WORKFLOW = path.join(
  __dirname,
  '..',
  '.github',
  'workflows',
...
function pinnedRef() {
  ...
  if (!m)
    throw new Error(
      `could not read the pinned ESPHome version from ${BUILD_WORKFLOW}`
    );
  return m[1];
}
...
function emittedKeys(source) {
  const keys = new Set();
  for (const m of source.matchAll(/root\[ESPHOME_F\("([a-z_]+)"\)\]/g))
    keys.add(m[1]);
  return [...keys].sort();
}
...
if (isMain) {
  ...
  try {
    ...
    if (!res.ok)
      bail(`could not fetch web_server.cpp at ${ref}: HTTP ${res.status}`);
  ...
  const builderHash =
    builder && crypto.createHash('sha256').update(builder).digest('hex');
  ...
  if (update) {
    ...
    fs.writeFileSync(
      SNAPSHOT,
      JSON.stringify(
        { ...previous, ref, keys, idBuilder: builderHash },
        null,
        2
      ) + '\n'
    );
    console.log(
      `updated ${path.basename(SNAPSHOT)} from ${ref} (${keys.length} keys)`
    );
    process.exit(0);
  }
  ...
  if (added.length)
    console.log(`note: new event keys at ${ref}: ${added.join(', ')}`);
  if (removed.length)
    console.log(`note: event keys gone at ${ref}: ${removed.join(', ')}`);
  ...
  console.log(
    `✓ web_server event contract unchanged at ${ref} (${keys.length} keys)`
  );
}
